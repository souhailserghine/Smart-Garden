...
include_once '../../controller/capteurC.php';
...
// Récupérer les capteurs avec données en temps réel
$capteurs = $capteurC->filterCapteurs();
...
foreach($tachesUtilisateur as $t) {
    ...
    if($t['priorite'] == 'Élevée' || $t['priorite'] == 'Elevée' || $t['priorite'] == 3) $taches_priorite_haute++;
...
<head>
    ...
    <style>
        ...
        .sensor-card-footer i {
            font-size: 14px;
            margin-right: 4px;
        }

        .last-update, .sensor-location {
            display: flex;
            align-items: center;
        }
...
-->
                ...
                <div class="row newsfeed-right-side-content mt-3">
                    ...
                    <div class="col-md-9 second-section" id="page-content-wrapper" style="padding: 20px;">
                        ...
                        <div class="hero-banner-improved">
                            ...
                            <p>Track your plants, monitor sensors, and manage tasks all in one place</p>
                        </div>
                        ...
                        <div class="container-fluid pt-2 px-0">
                            ...
                            <div class="stats-grid mb-4">
                                ...
                                <div class="stat-card-modern">
                                    ...
                                    <div class="stat-card-label">Total Plantes</div>
                                </div>

                                <!-- Plantes ajoutées aujourd'hui -->
                                <div class="stat-card-modern">
                                    <div class="stat-card-icon">🌿</div>
                                    <div class="stat-card-value"><?= $plantesToday ?></div>
                                    <div class="stat-card-label">Plantes Aujourd'hui</div>
                                </div>

                                <!-- Total Tâches -->
                                <div class="stat-card-modern">
                                    <div class="stat-card-icon">📋</div>
                                    <div class="stat-card-value"><?= $totalTaches ?></div>
                                ...
                                <div class="stat-card-modern">
                                    <div class="stat-card-icon">📅</div>
                                    <div class="stat-card-value"><?= $tachesToday ?></div>
                                    <div class="stat-card-label">Tâches Aujourd'hui</div>
                                </div>
                                ...
                                <div class="stat-card-modern">
                                    ...
                                    <div class="stat-card-label">Plantes Saines</div>
                                </div>

                                <!-- Plantes état moyen -->
                                <div class="stat-card-modern">
                                    <div class="stat-card-icon">⚠️</div>
                                    <div class="stat-card-value"><?= $plantes_etat_moyen ?></div>
                                    <div class="stat-card-label">Attention Requise</div>
                                </div>
                            </div>
                        </div>
                        <!-- Statistiques End -->
                        ...
                        <div class="container-fluid px-0 mb-4">
                            <div class="sensor-data-header mb-3 d-flex justify-content-between align-items-center">
                                <h5 class="mb-0" style="color: var(--text-dark); font-weight: 700;">
                                    <i class='bx bx-chip' style="color: #667eea;"></i> Données Capteurs en Temps Réel
                                </h5>
                                <a href="listCapteur.php" class="btn btn-sm btn-outline-primary rounded-pill">
                                    <i class='bx bx-trending-up'></i> Voir tous les capteurs
                                </a>
                            </div>
                            <div class="sensor-cards-grid">
                                <?php
                                $activeSensors = array_filter($capteurs, function($c) {
                                    return isset($c['temperature']) || isset($c['humidite']);
                                });
                                
                                if (count($activeSensors) == 0) {
                                    echo '<div class="col-12 text-center py-4">';
                                    echo '<i class="bx bx-error-circle" style="font-size: 48px; color: #cbd5e0;"></i>';
                                    echo '<p class="text-muted mt-2">Aucune donnée de capteur disponible</p>';
                                    echo '<small class="text-muted">Les capteurs commenceront à envoyer des données bientôt</small>';
                                    echo '</div>';
                                } else {
                                    foreach ($activeSensors as $capteur):
                                        $hasTemp = isset($capteur['temperature']) && $capteur['temperature'] !== null;
                                        $hasHumidity = isset($capteur['humidite']) && $capteur['humidite'] !== null;
                                        
                                        if (!$hasTemp && !$hasHumidity) continue;
                                ?>
                                <div class="sensor-card-realtime">
                                    <div class="sensor-card-header">
                                        <div class="sensor-card-title">
                                            <i class='bx bx-chip'></i>
                                            <span>Capteur #<?= $capteur['id_capteur'] ?></span>
                                        </div>
                                        <span class="sensor-status-badge active">
                                    ...
                                    <div class="sensor-readings">
                                        ...
                                        <div class="reading-item">
                                            ...
                                            <div class="reading-details">
                                                <span class="reading-label">Température</span>
                                                <span class="reading-value"><?= number_format($capteur['temperature'], 1) ?>°C</span>
                                            </div>
                                        </div>
                                        <?php endif; ?>
                                        ...
                                        <div class="reading-item">
                                            ...
                                            <div class="reading-details">
                                                <span class="reading-label">Humidité</span>
                                                <span class="reading-value"><?= number_format($capteur['humidite'], 1) ?>%</span>
                                            </div>
                                        </div>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <?php if (isset($capteur['last_reading'])): 
                                        $lastReading = new DateTime($capteur['last_reading']);
                                        $now = new DateTime();
                                        $diff = $now->diff($lastReading);
                                    ...
                                    <div class="sensor-card-footer">
                                        <span class="last-update">
                                            <i class='bx bx-time-five'></i> Il y a <?= $timeAgo ?>
                                        </span>
                                        <span class="sensor-location">
                                            <i class='bx bx-map'></i> <?= htmlspecialchars($capteur['emplacement'] ?? 'N/A') ?>
                                        </span>
                                    </div>
                                    <?php endif; ?>
                                </div>
                        ...
                        <!-- Mes Plantes Section -->
                         <div class="container-fluid p-4 mt-4 mb-4" style="max-width: 1400px; margin: 0 auto;">

                        
    <div class="mb-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h4 class="mb-2" style="color: var(--text-dark); font-weight: 700;">
                <i class='bx bx-leaf me-2' style="color: var(--primary-green);"></i>Mes Plantes
            </h4>
            <p class="text-muted small mb-0">Consultez vos plantes et leurs informations.</p>
        </div>
        <div class="d-flex gap-2">
            <button class="btn btn-success rounded-pill px-4" id="btnSuggestionPlante" title="Suggérer une plante" style="background: linear-gradient(135deg, var(--primary-green), var(--dark-green)); border: none;">
                <i class='bx bx-plus me-2'></i>Suggérer une plante
            </button>
            <button class="btn btn-outline-success rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#statsAdvancedModal" style="border-color: var(--primary-green); color: var(--primary-green);">
    ...
    if (count($mesPlantes) == 0) {
        ...
        echo '<div class="empty-state-icon"><i class="bx bx-leaf"></i></div>';
        echo '<h3 class="empty-state-title">No Plants Yet</h3>';
        echo '<p class="empty-state-text">Start your garden by adding your first plant</p>';
        echo '<button class="btn-modern btn-primary-modern" id="btnSuggestionPlante2">Add Your First Plant</button>';
        echo '</div>';
    } else {
        foreach($mesPlantes as $plante) {
            ...
            $temp = $plante['temperature'] ?? '22';
            $etat = $plante['etat_sante'] ?? 'Bon état';
            
            // Health badge class
            $healthClass = 'health-good';
            if (strpos($etat, 'Mauvais') !== false || strpos($etat, 'Poor') !== false) {
                $healthClass = 'health-poor';
            } elseif (strpos($etat, 'Moyen') !== false || strpos($etat, 'Medium') !== false) {
                $healthClass = 'health-medium';
            }

            // Image
            $img = !empty($plante['image']) ? $plante['image'] : '../image/plant.png';
            $planteJson = json_encode($plante, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_AMP);

            // Modern Plant Card
            echo '<div class="plant-card-modern" data-plante=\''.$planteJson.'\'>';
            
            // Image Container
            echo '<div class="plant-image-container">';
            echo '<img src="'.$img.'" alt="'.htmlspecialchars($plante['nom_plante']).'">';
            echo '<span class="plant-health-badge '.$healthClass.'">'.htmlspecialchars($etat).'</span>';
            echo '</div>';
            
            // Plant Info
            echo '<div class="plant-info">';
            echo '<h3 class="plant-name">'.htmlspecialchars($plante['nom_plante']).'</h3>';
            
            echo '<div class="plant-details">';
            echo '<div class="plant-detail-item">';
            echo '<i class="bx bx-droplet"></i>';
            echo '<span>'.$hum.'% Humidity</span>';
            echo '</div>';
            
            echo '<div class="plant-detail-item">';
            echo '<i class="bx bx-thermometer"></i>';
            echo '<span>'.$temp.'°C</span>';
            echo '</div>';
            
            echo '<div class="plant-detail-item">';
            echo '<i class="bx bx-water"></i>';
            echo '<span>'.($plante['besoin_eau'] ?? 'N/A').' ml/day</span>';
            echo '</div>';
            
            echo '<div class="plant-detail-item">';
            echo '<i class="bx bx-calendar"></i>';
            echo '<span>'.date('M d, Y', strtotime($plante['date_ajout'])).'</span>';
            echo '</div>';
            echo '</div>';
            
            // Actions
            echo '<div class="plant-actions">';
            echo '<button class="btn-modern btn-primary-modern btnViewPlante" data-id="'.$plante['id_plante'].'">View</button>';
            echo '<button class="btn-modern btn-secondary-modern btnEditPlante" data-id="'.$plante['id_plante'].'">Edit</button>';
            echo '</div>';
            
            echo '</div>'; // plant-info
            echo '</div>'; // plant-card-modern
        }
    }
    ?>
...
<!-- Modal Details Plante -->
<div class="modal fade" id="planteDetailsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content rounded-4 border-0 shadow-lg">
                ...
                <div class="row align-items-center">
                    <div class="col-md-5 text-center mb-4 mb-md-0">
                        <div class="position-relative d-inline-block">
                            <img id="detailImage" src="" class="rounded-4 shadow-sm" style="width: 240px; height: 240px; object-fit: cover;">
                        </div>
                    </div>
                    <div class="col-md-7">
                        <h2 id="detailNom" class="fw-bold mb-2 text-dark display-6"></h2>
                        <p class="text-muted mb-4"><i class="bx bx-calendar me-2"></i><span id="detailDate"></span></p>
                        ...
                        <div class="row g-4">
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <span class="text-secondary d-block text-uppercase fw-bold" style="font-size: 0.9rem;">Humidité</span>
                                        <span id="detailHumidite" class="fs-3 fw-bold text-dark"></span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    <div class="me-3">
                                        <i class="bx bxs-thermometer fs-1 text-primary"></i>
                                    </div>
                                    <div>
                                        <span class="text-secondary d-block text-uppercase fw-bold" style="font-size: 0.9rem;">Température</span>
                                        <span id="detailTemperature" class="fs-3 fw-bold text-dark"></span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    <div class="me-3">
                                        <i class="bx bx-water fs-1 text-primary"></i>
                                    </div>
                                    <div>
                                        <span class="text-secondary d-block text-uppercase fw-bold" style="font-size: 0.9rem;">Besoin Eau</span>
                                        <span id="detailEau" class="fs-3 fw-bold text-dark"></span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <span class="text-secondary d-block text-uppercase fw-bold" style="font-size: 0.9rem;">Santé</span>
                                        <span id="detailEtat" class="fs-3 fw-bold text-dark"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer border-0 justify-content-center pb-4">
                <button type="button" class="btn btn-dark rounded-pill px-5" data-dismiss="modal">Fermer</button>
...
function showPlanteDetails(plante) {
    console.log('Displaying plant details:', plante);
    
    document.getElementById('detailNom').textContent = plante.nom_plante || 'N/A';
    document.getElementById('detailDate').textContent = 'Ajouté le ' + (plante.date_ajout || 'N/A');
    document.getElementById('detailHumidite').textContent = (plante.niveau_humidite || '0') + '%';
    document.getElementById('detailTemperature').textContent = (plante.temperature || 'N/A') + '°C';
    document.getElementById('detailEau').textContent = (plante.besoin_eau || '0') + ' ml';
    document.getElementById('detailEtat').textContent = plante.etat_sante || 'N/A';
    
    const imgPath = plante.image ? plante.image : '../image/plant.png';
    document.getElementById('detailImage').src = imgPath;
    
    var myModal = new bootstrap.Modal(document.getElementById('planteDetailsModal'));
    myModal.show();
}
...
document.addEventListener('DOMContentLoaded', function() {
    // View buttons
    document.querySelectorAll('.btnViewPlante').forEach(button => {
        button.addEventListener('click', function() {
            const planteId = this.getAttribute('data-id');
            console.log('Fetching plant with ID:', planteId);
            
            // Fetch plante details and show modal
            fetch('getPlantes.php?id=' + planteId)
                .then(response => response.json())
                .then(plante => {
                    console.log('Received plant data:', plante);
                    if (plante && !plante.error) {
                        showPlanteDetails(plante);
                    } else {
                        console.error('Error or no data:', plante);
                        alert('Erreur lors du chargement des détails de la plante');
                    }
                })
                .catch(error => {
    ...
    // Edit buttons
    document.querySelectorAll('.btnEditPlante').forEach(button => {
        button.addEventListener('click', function() {
            const planteId = this.getAttribute('data-id');
            // Redirect to edit page
            window.location.href = 'modifierPlante_fo.php?id=' + planteId;
        });
    });
});
</script>
...
<div class="container-fluid pt-4 px-4">
    <div class="bg-light rounded-4 shadow-sm p-4">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h5 class="fw-bold mb-0">Gestion des Taches</h5>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 shadow">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-secondary rounded-pill" data-dismiss="modal">Fermer</button>
        ...
        <div class="modal-content border-0 shadow-lg" style="background: linear-gradient(135deg, rgba(255,255,255,0.98), rgba(245,247,255,0.98)); border-radius: 15px; overflow: hidden;">
            ...
            <div class="modal-body p-5" style="background: linear-gradient(135deg, rgba(255,255,255,0.98), rgba(245,247,255,0.98));">
                <form id="suggestionForm" enctype="multipart/form-data">
                    ...
                    <!-- Niveau d'humidit? avec slider -->
                    <div class="mb-5">
                        <label class="form-label fw-600" style="color: #333; font-size: 0.95rem; display: flex; align-items: center; margin-bottom: 12px;">
                            <i class='bx bx-water' style="color: #667eea; margin-right: 8px; font-size: 1.1rem;"></i>Niveau d'humidit?
                        </label>
                    ...
                    <div class="mb-3">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-top: 12px;">
                            <label style="cursor: pointer; padding: 14px 12px; border: 2px solid #e0e0e0; border-radius: 10px; text-align: center; transition: all 0.3s ease; background: white; display: flex; flex-direction: column; align-items: center; gap: 6px;">
                                ...
                                <i class='bx bx-smile' style="color: #28a745; font-size: 1.4rem;"></i>
                            ...
                            <label style="cursor: pointer; padding: 14px 12px; border: 2px solid #e0e0e0; border-radius: 10px; text-align: center; transition: all 0.3s ease; background: white; display: flex; flex-direction: column; align-items: center; gap: 6px;">
                                ...
                                <i class='bx bx-meh' style="color: #ffc107; font-size: 1.4rem;"></i>
                            ...
                            <label style="cursor: pointer; padding: 14px 12px; border: 2px solid #e0e0e0; border-radius: 10px; text-align: center; transition: all 0.3s ease; background: white; display: flex; flex-direction: column; align-items: center; gap: 6px;">
                                ...
                                <i class='bx bx-sad' style="color: #dc3545; font-size: 1.4rem;"></i>
            ...
            <div class="modal-footer border-0 p-5" style="background: linear-gradient(135deg, rgba(255,255,255,0.98), rgba(245,247,255,0.98)); border-radius: 0 0 15px 15px; gap: 10px; border-top: 1px solid #e0e0e0;">
                <button type="button" class="btn btn-outline-secondary rounded-pill px-5" data-dismiss="modal" style="border: 2px solid #ddd; color: #666; transition: all 0.3s; font-weight: 500;">Annuler</button>
        ...
        <div class="modal-content border-0 shadow-lg" style="background: linear-gradient(135deg, rgba(255,255,255,0.98), rgba(245,247,255,0.98)); border-radius: 15px; overflow: hidden;">
            ...
            <div class="modal-body p-5" style="background: linear-gradient(135deg, rgba(255,255,255,0.98), rgba(245,247,255,0.98));">
                <form id="suggestionTacheForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-5">
                        <label class="form-label fw-600" style="color: #333; font-size: 0.95rem; display: flex; align-items: center;">
                            <i class='bx bx-time-five' style="color: #17a2b8; margin-right: 8px; font-size: 1.1rem;"></i>Prochaine execution
                        </label>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-600" style="color: #333; font-size: 0.95rem; display: flex; align-items: center;">
                            <i class='bx bx-trending-up' style="color: #17a2b8; margin-right: 8px; font-size: 1.1rem;"></i>Priorite
                        </label>
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-top: 12px;">
                            <label style="cursor: pointer; padding: 14px 12px; border: 2px solid #e0e0e0; border-radius: 10px; text-align: center; transition: all 0.3s ease; background: white; display: flex; flex-direction: column; align-items: center; gap: 6px;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <div class="detail-card p-3 rounded-3" style="background: #f8f9fa; border-left: 4px solid #764ba2;">
                            <small class="text-muted d-block mb-2">
                                <i class='bx bx-package'></i> Quantit?
                            </small>
            ...
            <div class="modal-footer border-0 p-4" style="background: #f8f9fa;">
                <button type="button" class="btn btn-secondary rounded-pill px-4" data-dismiss="modal">Fermer</button>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="col-md-6">
                        <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <small class="text-light opacity-75">Tâches complétées</small>
...
<style>
    ...
    document.getElementById('detailStatut').textContent = (tache.estComplete == 1) ? '✓ Complétée' : 'En attente';
    ...
    console.log('Chart disponible:', typeof Chart);
        ...
        if (typeof Chart === 'undefined') {
            console.log('Chart.js n\'est pas encore charg?');
            return;
        }
    ...
    // Graphique Priorité des tâches
        try {
            ...
            console.error('Erreur graphique Priorit?:', e);
        }
    ...
    console.log('Modal trouvé:', statsModal ? 'Oui' : 'Non');
    console.log('Bootstrap disponible:', typeof bootstrap !== 'undefined' ? 'Oui' : 'Non');
        ...
        if (btnStats && statsModal && typeof bootstrap !== 'undefined') {
            btnStats.addEventListener('click', function(e) {
                console.log('Bouton stats cliqu? - affichage du modal');
                e.preventDefault();
        ...
        if (calendarEl) {
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    document.getElementById('detailStatut').textContent = (props.complete == 1) ? '✓ Complétée' : 'En attente';
    ...
    <script>
        ...
        function initSuggestionForm() {
            ...
            if (btnSuggestion && suggestionModal) {
                btnSuggestion.addEventListener('click', function(e) {
                    e.preventDefault();
                    console.log('Bouton suggestion cliqu? - ouverture du modal');
        ...
        setInterval(function() {
            const sensorCards = document.querySelector('.sensor-cards-grid');
            if (sensorCards) {
                // Reload only the sensor section without refreshing the whole page
                fetch(window.location.href)
                    .then(response => response.text())
                    .then(html => {
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const newSensorCards = doc.querySelector('.sensor-cards-grid');
                        if (newSensorCards) {
                            sensorCards.innerHTML = newSensorCards.innerHTML;
                        }
                    })
